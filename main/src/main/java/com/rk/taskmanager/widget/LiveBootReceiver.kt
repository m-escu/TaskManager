...
/**
 * Restores the permanent live-monitor notification after a reboot. The QS
 * tile session is intentionally NOT restored: tiles are user-momentary, the
 * "permanent notification" setting is the only source that survives restarts.
 *
...
class LiveBootReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (!Settings.permanentNotification) return
